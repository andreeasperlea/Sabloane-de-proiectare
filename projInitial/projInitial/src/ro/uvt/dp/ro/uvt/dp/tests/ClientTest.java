...
public class ClientTest {
    ...
    public void testAddAccountIncreasesList() {
        Client c = new Client.Builder("Andrei", "Timisoara")
                .addAccount(new BasicAccountFactory(), new RONInterestCalculator(), "RO002", 500.0)
                .build();
    ...
    public void testAddAccountExceedsMaxThrows() {
        ...
        for (int i = 1; i <= Client.NUMAR_MAX_CONTURI; i++) {
            builder.addAccount(new BasicAccountFactory(), new RONInterestCalculator(), "ACC" + i, 100.0);
        }

        assertEquals(Client.NUMAR_MAX_CONTURI, builder.build().getAccounts().size());

        assertThrows(IllegalStateException.class,
                () -> builder.addAccount(new BasicAccountFactory(), new RONInterestCalculator(), "ACC6", 100.0));
    }

    @Test
    public void testGetAccountReturnsCorrectAccount() {
        Client c = new Client.Builder("Andrei", "Timisoara")
                .addAccount(new SavingsAccountFactory(), new RONInterestCalculator(), "RO002", 500.0)
                .addAccount(new InvestmentAccountFactory(), new EURInterestCalculator(), "RO003", 600.0)
                .build();
    ...
    public void testGetAccountNotFoundThrows() {
        Client c = new Client.Builder("Andrei", "Timisoara").build();
    ...
    public void testToStringContainsClientInfo() {
        Client c = new Client.Builder("Andrei", "Timisoara")
                .addAccount(new InvestmentAccountFactory(), new RONInterestCalculator(), "RO002", 500.0)
                .build();
